...
config.best_save_dir = os.environ.get(
    "FFSR_V2_MODEL",
    rf"{ROOT}\Weights\FFSR_PointNet\best_shouxi_v2_backbone.pth",
)
config.save_root = os.environ.get(
    "FFSR_V2_SAVE_ROOT",
    rf"{ROOT}\Results\FFSR_PointNet\Shouxi_820_v2",
)
...
def predict_model(inputs: torch.Tensor, hr_priors: torch.Tensor) -> torch.Tensor:
    """Wrapper function for model inference with gradient disabled."""
    with torch.no_grad():
        return model(inputs, hr_priors)
...
model_name = "FFSR-PointNet-v2-backbone"
...
# Main Inference Loop
# =========================================================
for i, (inputs, targets, hr_priors, coords_low, coords_high) in enumerate(test_loader):
    ...
    with torch.no_grad():
        ...
        timer = timeit.Timer(lambda: predict_model(inputs, hr_priors))
        execution_time = timer.timeit(number=1)
        print(f"Sample {i} inference time: {execution_time:.6f} seconds")
        outputs = predict_model(inputs, hr_priors)
    ...
    ori_features[:, 0] = ori_features[:, 0] * std_depth + mean_depth
    ori_features[:, 1] = ori_features[:, 1] * std_velocity + mean_velocity
