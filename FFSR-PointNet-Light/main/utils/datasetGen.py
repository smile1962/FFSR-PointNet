...
class FlowFieldDataset(Dataset):
    def __init__(
        ...
        return_hr_priors: bool = False,
        stat_events: list[str] = None
    ):
        """
        Args:
            ...
            sample_indices: Optional list of h5 indices to load; mutually exclusive
                with num_samples. Used to draw a reproducible random subset.
            ...
            return_hr_priors: If True, return normalized HR query priors
            stat_events: If provided, normalization statistics are computed only
                from these simulation events.
        ...
        self.return_hr_priors = return_hr_priors
        self.stat_events = set(stat_events) if stat_events else None

        if self.return_hr_priors and not self.normalize:
            raise ValueError("return_hr_priors requires normalized flow features.")
        ...
        self.low_keys = sorted(list(self.low_h5.keys()), key=_natural_key)
        self.high_keys = sorted(list(self.high_h5.keys()), key=_natural_key)
        if sample_indices is not None:
            sample_indices = [int(i) for i in sample_indices]
            if any(i < 0 or i >= len(self.low_keys) for i in sample_indices):
                raise ValueError("sample_indices out of range")
            self.low_keys = [self.low_keys[i] for i in sample_indices]
            self.high_keys = [self.high_keys[i] for i in sample_indices]
        ...
        self.coord_min = high_sample[:, :2].min(axis=0).astype(np.float32)
        self.coord_max = high_sample[:, :2].max(axis=0).astype(np.float32)

        # Load metadata before normalization statistics so event labels can
        # define a leakage-free training distribution.
        self.meta_list = self._load_meta_from_h5(self.low_h5, len(self.low_keys))
        self.meta_list = self.meta_list[:len(self.low_keys)]
        self.meta_list = self.meta_list[:len(self.low_keys)]
        self.meta_list = self.meta_list[:len(self.low_keys)]
        self.high_meta_list = self._load_meta_from_h5(self.high_h5, len(self.low_keys))
        self.high_meta_list = self.high_meta_list[:len(self.low_keys)]

        if len(self.meta_list) != len(self.low_keys):
            alt = self.high_meta_list
        ...
        if len(self.high_meta_list) == len(self.meta_list):
            for low_meta, high_meta in zip(self.meta_list, self.high_meta_list):
                for key, value in high_meta.items():
                    low_meta.setdefault(key, value)
    ...
    def _compute_mean_std(self) -> tuple:
        ...
        for idx, (lk, hk) in enumerate(zip(self.low_keys, self.high_keys)):
            if self.stat_events is not None:
                event = self.meta_list[idx].get("event")
                if event not in self.stat_events:
                    continue
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        if self.return_hr_priors:
            coord_scale = self.coord_max - self.coord_min + 1e-8
            coords_norm = (high_tensor[:, :2] - self.coord_min) / coord_scale
            local_flow = torch.clamp(low_features[:, :2], -3.0, 3.0)
            hr_priors = torch.cat(
                [
                    coords_norm,
                    low_features[:, 2:3],
            ...
            hr_priors = hr_priors.to(low_features.dtype)
            to_return.append(hr_priors)
...
def split_by_events(
    ...
    """Split samples by simulation-event labels instead of random time steps."""

    train_events = set(train_events)
    val_events = set(val_events)
    overlap = train_events & val_events
    if overlap:
        raise ValueError(f"Train and validation events overlap: {sorted(overlap)}")

    train_idx = [
        idx
        for idx, meta in enumerate(dataset.meta_list)
        if meta.get("event") in train_events
    ]
    val_idx = [
        idx
        for idx, meta in enumerate(dataset.meta_list)
        if meta.get("event") in val_events
    ]

    if not train_idx or not val_idx:
        raise ValueError(
            f"Event split is empty: train={len(train_idx)}, val={len(val_idx)}."
        )
