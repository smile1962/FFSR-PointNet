...
class Config:
    ...
    result_dir_shouxi = r"G:\Projects\ShouXi\python\resulting_sanjiang_paperUsed"
    weight_dir = r"G:\Projects\ShouXi\python\weights"
    low_h5 = "LR_San_val.h5"  # low-resolution data file (already bilinearly interpolated)
    high_h5 = "HR_San_val.h5"  # high-resolution data file
    meta_path = "metadata_val.mat"  # metadata file

    # -------------------------- Dataset split --------------------------
    split_ratios = [0, 1, 0]  # [train, test, val] ratio
    random_seed = 1  # random seed (keeps the split consistent)

    # -------------------------- Output control --------------------------
    save_fig = True  # whether to save the visualization
